...
# 二  [服务端api说明](https://developers.weixin.qq.com/miniprogram/dev/server/API/user-login/api_code2session.html)
...
```php
     ...
     * 返回
     * array:2 [
     * "session_key" => "0LKBKW5ndDtHQ56M60aZAA=="
     * "openid" => "oH3Kq5V2jdSBZNz9xKbgvWhDwzKM"
     * ]
     */
    public function wxLogin($code)
    {
        try {
            if (empty($code)) throw new \Exception('code是必须的');
            $url = "https://api.weixin.qq.com/sns/jscode2session?appid={$this->appId}&secret={$this->appSecret}&js_code={$code}&grant_type=authorization_code";
            $res = json_decode(file_get_contents($url), true);
            if (!empty($res['errcode'])) throw new \Exception($res['errmsg'], $res['errcode']);
...
##  个人小程序极简登录实现（仅用 code → openid）

> **个人主体小程序，不需要获取用户昵称、头像等敏感信息，也不需要解密用户数据**，只需要通过微信登录 `code` 换取用户唯一标识 `openid`，就能完成登录逻辑，这是最简洁、最合规的个人小程序登录方案。

### 核心登录流程（极简版）

1. 前端调用 `wx.login()` 获取**临时登录凭证 code**（5 分钟有效期）
2. 前端把 code 传给你的后端服务器  
3. 后端拿着 code + 小程序的 `appid` + `appsecret` 请求微信接口   (**[后端请求-jscode2session](https://developers.weixin.qq.com/miniprogram/dev/server/API/user-login/api_code2session.html)**)
4. 微信接口返回当前用户的 **openid**（用户唯一标识，终身不变）
5. 后端查询 / 创建用户，返回登录态（token）给前端
6. 前端存储 token，后续请求携带 token 鉴权

###  **简单来说就是**

#### 1. 现在的登录流程 = 微信小程序标准「静默登录」
...
1. `uni.login` → 获取 code
2. 把 code 传给后端 → 后端去微信接口换 openid
3. 后端返回 token → 你存起来

**登录就完成了！**

#### 2. iv /encryptedData 是干嘛的？（历史遗留）

这两个是**老版本小程序**用来：

- 解密用户**完整信息**（手机号、unionid 等敏感数据）

但现在规则改了：

- **获取昵称头像不需要解密**
- **获取手机号有专门接口**
- **iv 和 encryptedData 现在基本废弃了**
...
##  **2022-11-08 之后**：无论个人 / 企业，**都不能再用 WXBizDataCrypt 解出头像昵称**微信开放社区

> 必须前端获取，但是手机号还是可以通过**WXBizDataCrypt **解密

## 个人登录code与获取手机号code区别

### 1. 先分清：两个完全不同的 code

① 登录 code（wx.login/uni.login）

- 来源：`uni.login()`
- 用途：**换 openid + session_key**（jscode2session）
- 特点：
  - 一次性
  - 用完就废
  - **只能用来登录，不能拿手机号**

② 手机号 code（getPhoneNumber）
...
- 来源：
  ...
  ```html
  <button open-type="getPhoneNumber" @getphonenumber="xxx">
  ```
...
- 用途：**调用 wxa/business/getuserphonenumber 拿手机号**
- 特点：
  - 一次性
  - 用完就废
  - **只能拿手机号，不能用来登录**

👉 **两个 code 完全独立、各用各的、互不消耗对方**。