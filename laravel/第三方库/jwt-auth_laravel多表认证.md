...
>
> 携带token查询用户id 再DB查询数据库返回用户的详细信息,依赖于Redis真实token存放value值（缺点 每次都需要根据token查询真实内容对服务器端的压力非常大）
>
...
###  添加中间件`auth:api` 之后接口很慢

> 观察每个接口都2秒以上  接口直接返回 `return 1` 也是达到了两秒  去掉中间件
>
> 发现接口只有200毫秒  所以问题出在了中间件上面经过排查 黑名单相关有点问题
...
为什么 `blacklist_enabled` = false 瞬间快了？

原理（一句话）

*JWT 黑名单 = 每次验证 token 都要读写数据库 / 缓存**，数据库 / 缓存慢 = 接口直接卡 2 秒。

详细说：

1. **blacklist_enabled = true**（默认）

   - 每次请求 `auth:api` 中间件
   - 都要去 **缓存 / 数据库** 查：这个 token 是否被拉黑（退出登录）
   - 你的缓存 / 数据库响应慢 → 直接卡 2 秒

2. **blacklist_enabled = false**
   ...
   3. 检查是否过期
   4. ↓↓↓ 这里就是瓶颈 ↓↓↓
   5. 检查 token 是否在【黑名单】里
   6. 通过 → 进入接口
   ```

   第 5 步就是 `blacklist_enabled` 控制的。
   ...
   try {
       ...
       // 2. 核心！检查是否在黑名单
       if ($this->auth->manager()->getBlacklist()->has($this->auth->getPayload())) {
   ...
   {
       // ↓↓↓ 这里每次请求都会查缓存 ↓↓↓
       return $this->storage->has($this->getKey($payload));
   }
   ```
   ...
   ## 1. `blacklist_enabled` 作用

   控制是否开启 **token 拉黑（强制登出）** 功能。

   ## 2. 底层原理

   - `true`：每次请求都**查缓存 / 文件 / 数据库** → 慢
   - `false`：跳过检查 → **超快**

   ## 3. 为什么你接口从 2s → 200ms

   因为关闭了**文件 / 数据库查询**，变成纯内存验证。
