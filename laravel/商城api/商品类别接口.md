...
CREATE TABLE `categories` (
  ...
  `is_directory` tinyint NOT NULL DEFAULT 0 COMMENT '是否为父目录 0=否 1=是 -- 1 表示这个分类下面还能挂子分类，是一个 “文件夹”；0 表示这个分类下面不能再挂子分类，只能挂商品，是 “最终分类”。',
  `level` tinyint NOT NULL DEFAULT 1 COMMENT '分类层级 1一级 2二级 3三级',
  `sort` int NOT NULL DEFAULT 0 COMMENT '排序',
  `small_img` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '小图标',
  `big_img` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '大图标',
  `root_id` bigint unsigned DEFAULT NULL COMMENT '所属一级分类ID(自动维护) 永远指向一级id',
   ...
   public function up()
      {
          Schema::create('categories', function (Blueprint $table) {
              ...
              $table->string('small_img')->nullable()->comment('小图标');
              $table->string('big_img')->nullable()->comment('大图标');
              $table->integer('root_id')->nullable()->comment('所属一级评论ID(自动维护)   永远指向一级id');
              $table->timestamps();
          });
   # 添加表注释       
...
public function index(Request $request)
{
    ...
    $categories = Categories::select(['id', 'pid', 'name', 'level'])
        ->when($status !== null, function ($query) use ($status) {
            $query->where('status', $status);
        })
        ->where('pid', 0) // 只查一级分类
        ->orderBy('id', 'asc') // 分类必须排序
        ->with([
            // 二级分类 + 状态过滤
            'children' => function ($query) use ($status) {
                $query->select(['id', 'pid', 'name', 'level'])
                    ->when($status !== null, function ($query) use ($status) {
                        $query->where('status', $status);
                    })
                    ->orderBy('id', 'asc');
...
class CategoryService extends CommonService
{
    public function create($params)
    {
        ...
        $name = $params['name'] ?? "";
        $pid = $params['pid'] ?? 0;
        $status = $params['status'] ?? Category::STATUS_1;
        $sort = $params['sort'] ?? 0;
        $smallImg = $params['small_img'] ?? null;
        $bigImg = $params['big_img'] ?? null;

        if (!empty($id)) $category = Category::query()->find($id);
        ...
        try {
            ...
            # 如果等级父级id为0 则自己是一级分类,反之则正好是父级等级+1是自己的等级
            $category->level = $pid == 0 ? 1 : Category::find($pid)->level + 1;
            $category->sort = $sort;
            $category->small_img = $smallImg;
            $category->big_img = $bigImg;
            $category->save();
            $category->root_id = self::getRootId($pid, $category->id);
            $category->save();
            \DB::commit();
     ...
     * 获取root_id
     * 思路:如果pid==0 就是自己的id
     * 如果pid!=0 取pid对应的root_id
     */
    public static function getRootId($pid, $id)
    {
        if ($pid == 0) return $id;
        $category = Category::query()->find($pid);
        return $category->root_id;
    }
}
```
