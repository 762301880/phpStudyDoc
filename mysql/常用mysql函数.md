...
> **MySQL 3.23 以上就支持** 此语法
>
> 实际开发中：**MySQL 5.0 / 5.1 / 5.5 / 5.6 / 5.7 / 8.0 全都完美支持**

| 名称          | 地址                                                      |
| ------------- | --------------------------------------------------------- |
| mysql高阶函数 | [link](https://www.runoob.com/mysql/mysql-functions.html) |

### **使用案例**

```mysql
CASE 
    WHEN l.money_type = 1 THEN '全款'
    WHEN l.money_type = 2 THEN '定金'
    WHEN l.money_type = 3 THEN '尾款'
    WHEN l.money_type = 4 THEN '退款'
    ELSE '其他'
END AS "款项类型"
```
...
2. **数据类型**：`THEN` 后的返回值类型建议统一（都是字符串 / 都是数字）
3. **ELSE 可选**：不写 ELSE 时，无匹配值会返回 `NULL`
4. **END 必写**：CASE 表达式必须以 `END` 收尾
5. **别名**：`AS` 可以省略，直接写 `END "款项类型"` 也生效

运行效果示例

假设 `l.money_type` 字段的值：
...
**单纯这个 CASE 表达式本身，几乎不影响性能**

真正影响速度的是：**有没有索引、全表扫描、关联查询的复杂度**

1. **CASE 是内存计算**

 只在 MySQL 查出数据后，在内存里做一次简单判断，开销微乎其微。

2. **真正拖慢的情况**

- 没有索引，导致 `SELECT * FROM 大表 WHERE ...` 全表扫描
- 关联了很多张大表
- 一次返回几十万、上百万行数据到应用端

3. **大数据量表怎么写才快？**

- 给 `money_type` 加索引（如果经常用它做查询条件）
- 不要 `SELECT *`，只查需要的字段
- 分页查询，不要一次拉全量数据
