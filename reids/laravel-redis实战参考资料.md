...
**结论：Redis 原生 Pub/Sub（发布订阅）不持久化消息；要持久化请用 Redis Streams（5.0+）。**

### 一、原生 Pub/Sub 的情况

- **不持久化、不落地磁盘**，消息只在内存中转、发完即弃。
- 只有**当前在线订阅者**能收到；订阅者离线、断线，消息**直接丢失**，重连也收不到历史消息。
- 无 ACK、无重试、无堆积，属于 “**即发即弃（fire-and-forget）**” 模型。

### 二、想持久化：用 Redis Streams（推荐）

Redis 5.0 引入的 **Stream** 是持久化的消息队列结构，替代 Pub/Sub 做可靠消息：

- ✅ **消息持久化**：数据落盘，受 RDB/AOF 保护，重启不丢。
- ✅ **消息可回溯**：按 ID / 时间重放历史消息。
- ✅ **消费者组 + ACK**：支持负载均衡、消息确认、重试、死信。
- ✅ **阻塞读取、消息堆积**：能力接近 Kafka/RabbitMQ。
...
- **Pub/Sub**：轻量广播，适合实时通知、日志推送，**不可靠、不持久化**。
- **Streams**：可靠消息队列，适合订单、通知、事件溯源，**持久化、可回溯、可重试**。
...
1. 每个元素有 **唯一成员 (member)** + **分数 (score)**
2. 自动按照 **score 排序**（升序 / 降序）
3. 支持快速获取：排名、分数、指定范围用户、总人数
4. 高并发下性能极强，千万级数据毫秒级响应
...
```bash
# 给用户 uid:1001 设置分数 980
ZADD game:rank 980 uid:1001

# 批量添加
ZADD game:rank 950 uid:1002 880 uid:1003
```

### 2. 增加分数（如签到、完成任务加分）

```bash
ZINCRBY game:rank 50 uid:1001  # uid:1001 加 50 分
```

**补充如何减分数**

```bash
ZINCRBY game:rank -5 uid:1001  # uid:1001 减 5 分
```



### 3. 获取**真实排名**（从 0 开始，+1 就是人类习惯的第 1 名、第 2 名）

```bash
# 降序（分数从高到低，排行榜默认用这个）
ZRANK game:rank uid:1001  # 返回数字，0=第1名，1=第2名...

# 如果你想直接返回从1开始的排名，代码里 +1 即可
```
