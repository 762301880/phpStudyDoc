...
**RDB** 和 **AOF** 两种持久化（备份）方式，核心区别一句话概括：

**RDB 是快照备份，快、体积小但丢数据多；AOF 是日志备份，安全、几乎不丢数据但文件大、恢复慢。**

##### 一、两种备份方式对比

1. RDB（Redis Database Backup）

- 原理：**定时把内存数据全量打成一个快照文件（dump.rdb）**
- 触发：
  - 手动执行 `SAVE` / `BGSAVE`
  - 配置规则：如 `save 60 1000` 60 秒内改 1000 次就备份
- 优点：
  - 文件小，备份快
  - 恢复速度极快
  - 对性能影响小
...
- 原理：**把每一条写命令追加到日志文件（appendonly.aof）**
...
| :--------: | :----------------------: | :--------------------: |
|  备份形式  |         全量快照         |     增量写命令日志     |
| 数据安全性 |    低，可能丢较多数据    |  高，最多丢 1 秒数据   |
|  恢复速度  |           极快           |          较慢          |
|  文件体积  |            小            |           大           |
|  性能影响  |            小            |          略大          |
|  适用场景  | 冷备、快速恢复、大数据量 | 追求数据安全、核心业务 |
...
| 名称              | 地址                                                         |
| ----------------- | ------------------------------------------------------------ |
| redis-lua脚本案例 | [link](https://redis.io/docs/latest/develop/programmability/eval-intro/) |
| 菜鸟教程-lua脚本  | [link](https://www.runoob.com/redis/redis-scripting.html)    |

####  为什么 Redis 要用 Lua 脚本？核心好处一句话总结

**把多个 Redis 命令打包成一个原子操作，一次性发给 Redis 执行，既保证安全，又大幅提升性能。**

Redis 使用 Lua 脚本的 4 大核心好处

1. **原子性（最重要！不会出现并发问题）**

Redis 执行 Lua 脚本时 **中间不会被其他命令打断**。
...
中间可能被别的请求插入，导致**数据错乱、超卖、计数错误**。

用 Lua 就安全：

整个逻辑打包成脚本，Redis **一次性执行完**，别人插不进来。

**作用：秒杀、库存扣减、点赞、排行榜、限流器 必须用 Lua 保证安全。**
...
可以**重复调用**，不用每次都发完整代码，效率更高。

4. **复杂逻辑可以在 Redis 端直接计算**
...
**Lua 脚本 = 把多个命令变成一个命令，原子执行、少网络、更安全、更强大。**

总结

1. **原子性**：命令不会被打断，并发安全
2. **减少网络**：一次请求执行多条命令，更快
3. **复杂逻辑**：能做 Redis 命令做不了的判断、计算
4. **复用性**：脚本可缓存，重复使用
