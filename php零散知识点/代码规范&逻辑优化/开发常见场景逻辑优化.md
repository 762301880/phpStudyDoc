...
```php
protected static function indexFields()
    {
        return [
            ['field' => 'id', 'title' => '主键', 'is_show' => true],
            ['field' => 'url', 'title' => '请求url', 'is_show' => true],
            ['field' => 'method', 'title' => '请求方式', 'is_show' => true],
            ['field' => 'line', 'title' => '报错行数', 'is_show' => true],
            ['field' => 'params', 'title' => '请求参数', 'is_show' => true],
            ['field' => 'file', 'title' => '异常文件完整路径', 'is_show' => true],
            ['field' => 'message', 'title' => '异常信息', 'is_show' => true],
            ['field' => 'ua', 'title' => 'User-Agent 用户代理', 'is_show' => true],
            ['field' => 'trace_as_string', 'title' => '异常堆栈信息', 'is_show' => false],
            ['field' => 'ip', 'title' => '客户端IP', 'is_show' => true],
            ['field' => 'http_status', 'title' => 'HTTP状态码', 'is_show' => true],
            ['field' => 'app_env', 'title' => '运行环境', 'is_show' => true],
            ['field' => 'exception_type', 'title' => '异常类名称', 'is_show' => true],
        ];
    }

    public function index(Request $request)
    {
        $params = $request->only(['created_at_start', 'created_at_end']);
        $limit = $request->input('limit', PaginateService::LIMIT_DEFAULT);

        $res = \App\Models\SystemExceptLog::where(function ($query) use ($params) {
            $start = $params['created_at_start'] ?? "";
            $end = $params['created_at_end'] ?? "";
            if (!empty($start) && !empty($end)) $query->whereBetween('created_at', [$start, $end]);
        })->orderBy('created_at', 'DESC')
            ->paginate($limit);
        PaginateService::appendDataToPaginate($res, ['fields' => self::indexFields()]);
        return $this->success('异常列表返回成功', $res);
    }
```
