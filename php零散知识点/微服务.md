...
## 分布式锁是每个 服务都要写一遍差不多的 RedisLockService 吗

**不用每个服务都各自重写一套完整 RedisLockService，但是有两种落地思路，看你的项目规模：**

1. **中小微多服务（你的这套 user/goods/order/pay）：抽成公共组件包（composer 包），所有服务引入同一个锁类，代码只写 1 份** 推荐
2. **超大型微服务集群：独立分布式锁服务（不推荐新手，太重）**

> 但是要分清一件事：**锁的逻辑代码可以复用；但【加锁的业务位置】是写在各自业务服务里面的，不能省。**

### 方案 1：公共 Composer 包

所有服务都是 Laravel 项目，非常适合。

1. 新建一个私有 composer 包，比如 `company/common`
2. 在包里写 `RedisLockService`，封装：
   - tryLock 获取锁（支持自动续期 / 看门狗）
   - releaseLock 释放锁
   - 防误删（锁带上唯一标识 uuid，只能自己释放自己的锁）
3. `user-service / goods-service / order-service / pay-service`，全部 `composer require company/common`
4. 各个服务直接 `app(RedisLockService::class)` 使用锁工具。

各个服务业务代码里**只调用这个工具**，不用重写锁底层：

```php
// goods-service 扣库存业务代码，业务层调用公共锁
$lock = app(RedisLockService::class);
$token = $lock->tryLock("stock:goods:{$goodsId}", 10);
if (!$token) throw new \Exception("抢购繁忙");
try {
    // 扣库存业务
} finally {
    $lock->releaseLock("stock:goods:{$goodsId}", $token);
}
```

优点：

- 锁底层代码只维护一份，修复 bug、升级看门狗，所有服务一次性更新
- 每个服务各自连接自己的 Redis 实例（或者共用同一个 Redis，看架构）

> 注意：锁用的 Redis，**可以共用一套 Redis，也可以每个服务独立 Redis**，工具类本身不关心。

缺点：

- 需要维护私有 composer 仓库（内部 git 仓库即可，简单）

### 方案 2：重复拷贝代码（不推荐）

直接把 RedisLockService.php 复制粘贴到每个服务。

问题：

改一处 bug，所有服务都要手动复制更新，版本不一致，某一个服务锁实现有 bug，线上事故。
