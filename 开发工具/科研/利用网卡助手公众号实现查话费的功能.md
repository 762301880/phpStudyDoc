...
<img src="https://s2.loli.net/2022/04/29/Sl1wnvW5VNiLXaY.png" alt="1651215792(1).jpg" style="zoom:50%;" />
...
class PhoneChargesNotice extends Command
{
    ...
    public function handle()
    {
        ...
        if ($balance <= 1) {
            $res = (new WorkController())->sendMessage('您的话费剩余:' . $balance . '请尽快充值');
        }
        $flowValue = $res['data']['balanceInfo']['flowValue'];//流量
        if ($flowValue <= 100) {
            $res = (new WorkController())->sendMessage('您的流量剩余:' . $flowValue . 'M,请看情况使用');
        }
        $voiceValue = $res['data']['balanceInfo']['voiceValue'];//剩余免费童话分钟
        if ($voiceValue <= 100) {
            $res = (new WorkController())->sendMessage('您的剩余免费童话时长:' . $voiceValue . '请看情况使用');
        }
    }
